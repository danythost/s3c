...
<div class="mb-6 flex items-center justify-between">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-400 font-mono">{{ $transaction->reference }}</p>
        </div>
    </div>

    @if($transaction->status !== 'success' && in_array($transaction->source, ['airtime', 'data']))
        <form action="{{ route('admin.orders.retry', $transaction->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to retry this transaction?')">
            @csrf
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="glass p-6 rounded-2xl">
            ...
            <div class="grid grid-cols-2 gap-y-4 text-sm">
                <div>
                    ...
                    @endphp
                    <span class="font-bold {{ $statusColors[$transaction->status] ?? 'text-gray-400' }} uppercase">{{ $transaction->status }}</span>
                </div>
                <div>
                    <label class="block text-gray-400 text-xs uppercase mb-1">Date</label>
                    <span class="text-white">{{ $transaction->created_at->format('M d, Y H:i:s') }}</span>
                </div>
                <div>
                    <label class="block text-gray-400 text-xs uppercase mb-1">Amount</label>
                    <span class="text-xl font-bold font-mono text-white">₦{{ number_format($transaction->amount, 2) }}</span>
                </div>
                <div>
                    <label class="block text-gray-400 text-xs uppercase mb-1">Service Type</label>
                    <span class="text-white uppercase">{{ $transaction->source ?? $transaction->type }}</span>
                </div>
                <div>
                    <label class="block text-gray-400 text-xs uppercase mb-1">Provider</label>
                    <span class="text-white font-mono text-xs p-1 bg-white/5 rounded">{{ $transaction->meta['provider'] ?? 'N/A' }}</span>
                </div>
            </div>
        </div>
        ...
        <!-- Service Details -->
        <div class="glass p-6 rounded-2xl">
            <h3 class="text-lg font-bold mb-4 border-b border-white/10 pb-2">Meta Details</h3>
            <div class="bg-[#0f172a] p-4 rounded-xl font-mono text-xs text-blue-300 overflow-x-auto">
                <pre>{{ json_encode($transaction->meta, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
            </div>
        </div>
        
        <!-- Logs -->
        @if(isset($transaction->meta['api_response']))
        <div class="glass p-6 rounded-2xl">
            <h3 class="text-lg font-bold mb-4 border-b border-white/10 pb-2">Request / Response Logs</h3>
            <div class="space-y-4">
                @foreach(is_array($transaction->meta['api_response']) ? $transaction->meta['api_response'] : [$transaction->meta['api_response']] as $index => $log)
    ...
    <div class="space-y-6">
        <div class="glass p-6 rounded-2xl">
            ...
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    <h4 class="font-bold text-white">{{ $transaction->user->name ?? 'Unknown' }}</h4>
                    <p class="text-xs text-gray-400">ID: {{ $transaction->user->id ?? '-' }}</p>
                </div>
            </div>
            <div class="space-y-3 text-sm">
                <div>
                    <label class="block text-gray-400 text-xs mb-1">Email</label>
                    <a href="mailto:{{ $transaction->user->email ?? '' }}" class="text-blue-400 hover:underline">{{ $transaction->user->email ?? '-' }}</a>
                </div>
                <div>
                    <label class="block text-gray-400 text-xs mb-1">Joined</label>
                    <span class="text-white">{{ $transaction->user ? $transaction->user->created_at->format('M Y') : '-' }}</span>
                </div>
                <div class="pt-4 mt-4 border-t border-white/10">
                    @if($transaction->user)
                    <a href="{{ route('admin.users.show', $transaction->user) }}" class="block w-full text-center px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-xs font-bold transition-all">
                        View User Profile
                    </a>
                    @endif
                </div>
